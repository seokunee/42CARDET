...
int main(int argc, char **argv) {
	...
	if (bind(sockfd, (const struct servaddr *)&servaddr, sizeof(servaddr)) < 0)
		ftError(NULL);
	...
	while(1) {
		...
		for (int fdI = 0; fdI <= fdMax; fdI++) {
			if (FD_ISSET(fdI, &readfds) && fdI == sockfd) {
				int connfd = accept(sockfd, (struct servaddr *)&servaddr, &len);
				if (connfd < 0)
					continue;
			...
			if (FD_ISSET(fdI, &readfds) && fdI != sockfd) {
				int res = recv(fdI, bufferRead, 66536, 0);
				if (res <= 0) {
